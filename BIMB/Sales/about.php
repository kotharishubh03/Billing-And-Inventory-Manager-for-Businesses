<?php
    ...
    if (isset($_POST['update'])) {
        ...
        if(empty($rows['cus_id'])){
            $stmt = $pdo->prepare('INSERT INTO `customers`(`cus_name`, `mobile_no`, `email`, `address`, `gstin`) VALUES (:cus_name,:mob_no,:cus_email,:cus_address,:cus_gstin)');
            $stmt->execute(array(':cus_name'=> $_POST['cus_name'],':mob_no'=> $_POST['cus_mob'],':cus_address'=> $_POST['cus_address'],':cus_email'=> $_POST['cus_email'],':cus_gstin'=> $_POST['cus_gstin']));
            $cus_id = $pdo->lastInsertId();
        } else {
            $cus_id = $rows['cus_id'];
        }

        $stmt = $pdo->prepare('UPDATE `sales` SET `cus_id`=:cus_id,`bill_no`=:bill_no,`sale_date`=:sale_date,`total`=:total,`discount`=:disc ,
            `pay_type`=(SELECT `pay_mode_id` FROM `payment_mode` WHERE `pay_mode`=:pay_mode),`pay_date`=:pay_date WHERE `sale_id`=:sale_id');
        $stmt->execute(array(':cus_id'=>$cus_id,':bill_no'=>$_POST['bill_no'],':sale_date'=> date("Y-m-d", strtotime($_POST['pur_date'])),':total'=> $_POST['billtotal'],':disc'=> $_POST['disc'],':pay_mode'=> $_POST['paymode'],':pay_date'=> date("Y-m-d", strtotime($_POST['pay_date'])),':sale_id'=>$_POST['update']));

        
        $stmt = $pdo->prepare('DELETE FROM `sales_product` WHERE sale_id=:sale_id');
        $stmt->execute(array(':sale_id'=>$_POST['update']));

        for ($i=1;$i<20;$i++){
            $temp='prd_id_'.$i;
            if(isset($_POST[$temp])){
                if ($_POST['subtotal_'.$i]>0){
                    $stmt = $pdo->prepare('INSERT INTO `sales_product`(`sale_id`, `prd_id`, `qnt`, `sell_price`) VALUES (:sale_id,:prd_id,:qnt,:sp)');
                    $stmt->execute(array(':sale_id'=>$_POST['update'],':prd_id'=>$_POST[$temp],':qnt'=> $_POST['Qnt_'.$i],':sp'=> $_POST['sp_'.$i]));
                }
            }
            else{
                break;
        ...
        $_SESSION['success']="Successfully Updated ! Add Another Sale";
        header("Location: ./about.php?sale_id=".$_POST['update']);
        return;
    }
                    ...
                    <form name='addnew' method="post">
                        <div class="w3-row">
                            ...
                            <div class="w3-col m5 w3-padding ">
                                ...
                                <input id="bill_no" class="w3-input" type="text" name="bill_no" size="30" value="<?=$billdetails['bill_no']?>" />
                            </div>
                            </div>
                        
                        <a onclick="myAccFunc1('cus_info')" class="w3-button w3-block w3-dark-grey">More Customer Info <i class="fa fa-caret-down"></i></a>
                        
                        <div id="cus_info" class="w3-hide w3-container ">
                        <div class="w3-row">
                            <div class="w3-col m5 w3-padding ">
                                <label for="cus_address">Customer Address :</label>
                                <input id="cus_address" class="w3-input" type="text" name="cus_address" size="30"/>
                            </div>
                            <div class="w3-col m5 w3-padding ">
                                <label for="cus_email">Customer Email :</label>
                                <input id="cus_email" class="w3-input" type="text" name="cus_email" size="30"/>
                            </div>
                            <div class="w3-col m5 w3-padding ">
                                <label for="cus_gstin">GSTIN.:</label>
                                <input class="w3-input" id="cus_gstin" name="cus_gstin" size="30" />
                            </div>
                        </div>
                        </div>
                        ...
                        <div class="w3-row">
                            ...
                            <div class="w3-col m2 w3-padding ">
                                <input id="prd_id" class="w3-input" type="text" name="" size="6" placeholder="product id" tabindex="7" />
                            </div>
                            <div class="w3-col m2 w3-padding ">
                                ...
                                <table class="w3-table-all w3-small sk-table">
                                    ...
                                    <tbody id="additemdiv" >
                                        ...
                                        foreach($billitems as $item){
                                            ...
                                            <td><input id="subtotal_'.$countitem.'" name="subtotal_'.$countitem.'" class="w3-input" type="text" size="6" value="'.$item['amt'].'"/> </td>
                                            </tr>');
                                            $countitem++;
                                        }
                                        ?>
                        ...
                        <button class="w3-button w3-left w3-margin-bottom w3-dark-grey" style="width:49%" type="submit" name="update" value="<?=$_GET['sale_id']?>" >Update </button>
                        <a class="w3-button w3-right w3-margin-bottom w3-dark-grey" style="width:49%" href='./print.php?sale_id=<?=$_GET['sale_id']?>' target="_newtab">Print</a>
                    </form>
                </div>
            </div>
        </div>
        ...
        $(document).ready(function () {
            ...
            $("#addproduct").click(function(){
                ...
                if (itemname=="" || itemqnt=="" || productid==""){
                    ...
                    <td><input id="subtotal_'+countitem+'" name="subtotal_'+countitem+'" class="w3-input" type="text" size="6" value="'+temp+'"/> </td> \
                    </tr>');
                    tabindex++;
